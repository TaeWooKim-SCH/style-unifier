...
> 2D 벡터/일러스트 게임 에셋을 reference 이미지 기준으로 스타일 통일하는 도구  
...
**인디 게임 개발자가 여러 소스에서 조달한 2D 벡터/일러스트 에셋들의 스타일을, reference 이미지 기준으로 자동 통일시키는 도구.**

### 1.2 핵심 제약 (순서 중요)

1. **형태 보존**: 캐릭터/오브젝트의 디자인 자체는 바꾸지 않는다.
2. **스타일만 이식**: 팔레트, 라인, 그림자, 텍스처만 reference에 맞춘다.
3. **도메인 특화**: 포토리얼이 아닌 벡터/일러스트 게임 에셋 대상.
...
본 프로젝트는 **이미지 입력 → 이미지 출력**의 vision pipeline으로, 전통적 CV 주제(Semantic Segmentation, Edge Detection, Color Clustering, Image Compositing, IQA)와 현대 CV 주제(Visual Representation Learning, Conditional Image Generation, Structure-Conditioned Generation)를 통합한다. 상세 매핑은 [Section 4](#4-vision-task-매핑) 참조.
...
| 애니메이션 프레임 일관성 | 시간축 일관성은 별개 연구 주제 |
| 배경/UI/이펙트 에셋 | 캐릭터 에셋만 우선 |
...
| 5 | 5.2.2 Multi-Reference 통합 | **Feature Aggregation / Fusion** | Mean / weighted pooling |
| 6 | 5.3 생성 파이프라인 | **Conditional Image-to-Image Translation** | SDXL Diffusion |
| 7 | 5.3 ControlNet 주입 | **Structure-Conditioned Generation** | ControlNet (lineart, depth) |
| 8 | 5.4.1 알파 복원 | **Image Compositing / Alpha Matting** | Feathering, Gaussian blur |
| 9 | 5.4.2 팔레트 양자화 | **Color Transfer / Palette Mapping** | Nearest-neighbor matching |
| 10 | 5.4.3 품질 검증 | **Perceptual Image Quality Assessment (IQA)** | LPIPS, CLIP sim, DINOv2 |
...
```
   ...
   Reference Image (RGB)
    ...
    │   - Source:    img2img input             │
    │   - Style:     IP-Adapter injection      │
    │   - Structure: ControlNet guide          │
    └──────────────────────────────────────────┘
                ↓
         Generated RGB Image
                ↓
    [Alpha Compositing]           ← Mask 복원
                ↓
    [Color Transfer]              ← 팔레트 정제
                ↓
    [Perceptual IQA]              ← 품질 평가
                ↓
         Output (RGBA Image)
```
...
#### 4.3.2 인코딩 단계 (Representation Learning)

**Visual Feature Extraction**

CLIP의 vision transformer가 이미지를 고차원 embedding으로 변환. Pretrained vision model을 **feature extractor**로 활용하는 전형적 패턴.
...
여러 reference의 feature를 하나로 통합. Multi-view learning, set representation의 한 형태.

- **CV 교과서 위치**: Multi-view Learning, Set Representation
- **관련 연구**: DeepSets (Zaheer et al. 2017), attention-based aggregation

#### 4.3.3 생성 단계 (Modern Generative Vision)

**Conditional Image-to-Image Translation**

본 프로젝트의 **핵심 CV task**. pix2pix(Isola et al. 2017), CycleGAN(Zhu et al. 2017) 시절부터 이어져 온 주류 CV 주제로, 현재는 diffusion model이 주도.

- **Input modality**: Image (source + reference + control signals)
- **Output modality**: Image (stylized output)
- **Sub-task**: Style Transfer with Structure Preservation

Style transfer 자체는 Gatys et al. (2015)의 Neural Style Transfer로 시작해 AdaIN, StyleGAN, 현대 diffusion 기반까지 CV의 장기 연구 주제.

- **CV 교과서 위치**: Generative Models, Image-to-Image Translation, Style Transfer
- **대표 논문**: pix2pix (2017), StyleGAN (2019), DDPM (2020), SDXL (2023), IP-Adapter (2023)

**Structure-Conditioned Generation**

일반 diffusion에 **구조 조건**(lineart, depth)을 추가 주입. Conditional generation + geometric reasoning의 결합.

- **대표 논문**: ControlNet (Zhang et al. 2023), T2I-Adapter (Mou et al. 2023)

#### 4.3.4 후처리 단계 (Image Processing + IQA)

**Image Compositing / Alpha Matting**

생성된 RGB에 원본 alpha mask를 합성. CV와 computer graphics의 경계 영역. Feathering을 위한 Gaussian blur는 고전 **image filtering**.

- **CV 교과서 위치**: Image Processing, Alpha Compositing
- **관련 기법**: Laplacian pyramid blending, Poisson blending

**Color Transfer**

Reference의 색 분포를 생성 결과에 이식. Reinhard et al. (2001)의 LAB 색공간 기반 기법부터 neural color transfer까지 이어지는 세부 주제.
...
생성 결과의 품질을 자동 평가. LPIPS는 VGG 기반 perceptual metric, DINOv2는 self-supervised ViT 기반 feature similarity. IQA는 CV의 독립된 세부 분야.
...
**전통적 CV**: Semantic Segmentation, Edge Detection, Color Clustering, Alpha Compositing, IQA (5개 영역)

**현대 CV**: Visual Representation Learning, Conditional Image Generation, Structure-Conditioned Generation (3개 영역)

**세부 전문 분야**: Style Transfer, Color Transfer, Image Matting, Feature Aggregation (4개 영역)
...
**학부 프로젝트로서의 포지셔닝**: 각 vision task를 from-scratch로 구현하지는 않지만, **여러 vision task를 통합하고 도메인(게임 에셋)에 최적화하는 system-level CV 연구**로 분류된다. 이는 현대 CV 학계의 주류 흐름(foundation model 활용 + 응용)과 일치한다.
...
#### 5.1.1 배경 분리 (bg_removal.py) — *Semantic Segmentation / Image Matting*
...
#### 5.1.2 Lineart 추출 (lineart.py) — *Edge Detection*
...
#### 5.1.3 팔레트 추출 (palette.py) — *Color Clustering*
...
#### 5.2.1 IP-Adapter Wrapper (ip_adapter_wrapper.py) — *Visual Feature Extraction*
...
#### 5.2.2 Multi-Reference 전략 (multi_ref.py) — *Feature Aggregation*
...
#### 5.3.1 메인 파이프라인 (pipeline.py) — *Conditional Image-to-Image Translation + Structure-Conditioned Generation*
...
#### 5.4.1 알파 복원 (alpha_restore.py) — *Image Compositing / Alpha Matting*
...
#### 5.4.2 팔레트 정제 (palette_quantize.py) — *Color Transfer*
...
#### 5.4.3 품질 검증 (quality_check.py) — *Perceptual Image Quality Assessment*
...
**문제**: "동일 캐릭터의 여러 스타일 버전" 같은 paired 데이터는 실제로 수집하기 매우 어렵다.
...
def dino_identity(source, generated, model="dinov2_vitl14"):
    """DINOv2 embedding cosine. 높을수록 캐릭터 identity 유지."""
    ...
...
각 실험에서 50샘플 생성 → 5개 메트릭 측정 → 평균/표준편차 기록.
...
**프로토콜**:
1. 10쌍의 변환 예시 제시 (A: baseline vs B: 본 시스템, 순서는 랜덤).
2. 각 쌍에 대해 다음 3개 질문:
   - "reference 스타일과 더 가까운 것은?" (style fidelity)
   - "원본 캐릭터 디자인을 더 잘 보존한 것은?" (content preservation)
...
**본인 개발 게임에 적용**:
- 현재 스타일 불일치가 있는 에셋 10-20장을 선정.
- 본 시스템으로 일괄 변환.
- Before/after 스크린샷 비교.
- 수정 없이 사용 가능한 비율, 수동 보정이 필요한 비율 측정.
...
**결정 근거**: 
- "동일 캐릭터의 여러 공식 스타일 버전"이라는 데이터는 존재하지 않음.
- 아티스트 협업은 시간/비용 부담.
...
**결정 근거**:
- Section 4에 명시된 바와 같이 본 프로젝트는 **10개의 명확한 vision task**(Semantic Segmentation, Edge Detection, Color Clustering, Feature Extraction, Feature Aggregation, Image-to-Image Translation, Structure-Conditioned Generation, Alpha Compositing, Color Transfer, IQA)를 통합한다.
...
| IP-Adapter가 스타일과 내용을 분리 못함 (reference의 캐릭터가 source에 섞임) | 중 | 고 | InstantStyle 방식(특정 레이어만 주입)으로 완화. 실패 시 IP-Adapter scale 낮추고 ControlNet 의존도 높임 |
...
*(개발 진행하면서 Open Questions에서 여기로 이동)*
...
*이 문서는 살아있는 문서다. 개발 진행에 따라 Open Questions, ADR, 주차별 체크리스트가 갱신된다.*
