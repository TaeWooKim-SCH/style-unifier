...
**Style Unifier는 이 문제를 해결합니다.** 기준이 되는 reference 이미지 한 장을 주면, 나머지 에셋들의 스타일을 그 기준으로 자동 변환합니다. **캐릭터 디자인은 그대로 보존**한 채 스타일만 이식합니다.
...
Vision pipeline으로 10개 task를 통합합니다:
...
| 인코딩 | Visual Feature Extraction        | CLIP ViT-H/14 (IP-Adapter) |
| 인코딩 | Feature Aggregation              | Multi-reference pooling    |
| 생성   | Conditional Image-to-Image       | SDXL + Animagine-XL-3.1    |
| 생성   | Structure-Conditioned Generation | ControlNet (lineart)       |
| 후처리 | Alpha Compositing                | Feathering + Mask restore  |
| 후처리 | Color Transfer                   | Palette quantization       |
| 평가   | Perceptual IQA                   | LPIPS, CLIP, DINOv2        |

핵심 차별점은 **형태 보존 강제**입니다. 범용 img2img 모델은 reference의 디자인을 섞어버리지만, 이 도구는 ControlNet으로 원본 구조를 강하게 제약해서 **디자인은 그대로, 스타일만 변환**을 달성합니다.
...
```bash
# 저장소 클론
git clone https://github.com/<your-username>/style-unifier.git
cd style-unifier
...
source = Image.open("character.png")
...
- 벡터/일러스트 스타일 2D 게임 제작자

### 현재 지원 범위

- ✅ 정적 캐릭터/오브젝트 이미지 (RGBA)
- ✅ 벡터 / 일러스트 스타일
- ❌ 픽셀 아트 _(도메인 밖. [Retro Diffusion](https://www.retrodiffusion.ai/) 참조)_
- ❌ 애니메이션 프레임 (향후 확장)
- ❌ 포토리얼 스타일 (도메인 밖)
...
@software{style_unifier_2026,
  ...
  year   = {2026},
  url    = {https://github.com/<your-username>/style-unifier}
}
```
